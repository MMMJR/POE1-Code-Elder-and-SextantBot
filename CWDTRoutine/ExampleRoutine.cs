...
namespace ExampleRoutine
{
    ...
    public class ExampleRoutine: IRoutine
    {
        ...
        public async Task<LogicResult> Logic(Logic logic)
        {
            ...
            if (logic.Id == "hook_combat")
            {
                ...
                while(!LokiPoe.Me.HasAura("Arcane Surge"))
                {
                    for (int x = 1; x < 5; x++)
                    {
                        ...
                        UseFlask(thisflask, x);
                        await Wait.SleepSafe(30);
                    }

                    LokiPoe.Input.SimulateKeyEvent(System.Windows.Forms.Keys.X, true, false, true);
                    await Wait.Sleep(60);
                    LokiPoe.Input.SimulateKeyEvent(System.Windows.Forms.Keys.X, true, false, true);
                    await Wait.Sleep(850);
                }
        ...
        public bool UseFlask(Item thisflask, int slot)
        {
            ...
            if (!LokiPoe.InGameState.QuickFlaskHud.UseFlaskInSlot(slot)) return false;
            return true;
        }
    ...
    public static class Wait
    {
        ...
        {
            if (await For(() => LokiPoe.StateManager.IsAreaLoadingStateActive, "loading screen", 100, 6000))
            {
