    ...
    public static class TravelHelper
    {
        public static async Task<bool> TravelToZone(DatWorldAreaWrapper datWorldAreaWrapper)
        {
            var ret = false;
                ...
                {
                    MmmjrBot.Log.DebugFormat("[MmmjrBot] Found Waypoint for {0}", datWorldAreaWrapper.Name);
                ...
                if (datWorldAreaWrapper.Connections.Any(x => x.Id == LokiPoe.CurrentWorldArea.Id))
                {
                    ...
                    if (zone != null)
                    {
                        MmmjrBot.Log.DebugFormat("[MmmjrBot] Found Areatransation for {0}", datWorldAreaWrapper.Name);
