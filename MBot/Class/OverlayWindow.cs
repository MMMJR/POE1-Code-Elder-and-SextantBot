...
namespace MmmjrBot.Class
{
	public class OverlayWindow : IDisposable
    {
		...
		public OverlayWindow(IntPtr parentPtr)
		{
			...
			{
				FPS = MmmjrBotSettings.Instance.FPS,
				IsTopmost = true,
				IsVisible = true
			};
		...
		private void _window_SetupGraphics(object sender, SetupGraphicsEventArgs e)
		{
            ...
            _brushes["transparent_black"] = gfx.CreateSolidBrush(0, 0, 0, MmmjrBotSettings.Instance.OverlayTransparency);
		...
		private void _window_DrawGraphics(object sender, DrawGraphicsEventArgs e)
		{
            var gfx = e.Graphics;
			gfx.ClearScene();
			if (!MmmjrBotSettings.Instance.EnableOverlay)
			{
				return;
			}

			if (!MmmjrBotSettings.Instance.DrawInBackground && LokiPoe.ClientWindowHandle != GetForegroundWindow())
			{
				return;
			}
            ...
            var botInfoText = new StringBuilder()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Follow: ", $"{MmmjrBotSettings.Instance.ShouldFollow}","")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Loot: ", $"{MmmjrBotSettings.Instance.ShouldLoot}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Attack: ", $"{MmmjrBotSettings.Instance.ShouldKill}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Sentinel: ", $"{MmmjrBotSettings.Instance.UseStalkerSentinel}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Auto Teleport: ", $"{!MmmjrBotSettings.Instance.DontPortOutofMap}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Follow Dist: ", $"{MmmjrBotSettings.Instance.FollowDistance}/{MmmjrBotSettings.Instance.MaxFollowDistance}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Combat Dist: ", $"{MmmjrBotSettings.Instance.MaxCombatDistance}", "")).AppendLine()
                .Append(string.Format("{0,-16}  {1,-10}  {2,5}", "Loot Dist: ", $"{MmmjrBotSettings.Instance.MaxLootDistance}", "")).AppendLine()
				.ToString();


			gfx.DrawTextWithBackground(_fonts["consolas"], _brushes["green"], _brushes["transparent_black"], 58, 20, infoText);
            gfx.DrawTextWithBackground(_fonts["consolas"], _brushes["green"], _brushes["transparent_black"], MmmjrBotSettings.Instance.OverlayXCoord, MmmjrBotSettings.Instance.OverlayYCoord, botInfoText);
			if (!LokiPoe.IsInGame) return;

            if (MmmjrBotSettings.Instance.DrawMobs || MmmjrBotSettings.Instance.DrawCorpses)
				DrawMobs(gfx);
        }
			...
			foreach (var monst in monsts)
			{
				foreach (var networkObject in monst.Value)
				{
					try
					{
						...
						if (monter.IsActiveDead && !MmmjrBotSettings.Instance.DrawCorpses) continue;
						if (!monter.IsActiveDead && !MmmjrBotSettings.Instance.DrawMobs) continue;
        ...
        public void SetTransparency(int overlayTransparency)
        {
			...
			_brushes["transparent_black"] = _window.Graphics.CreateSolidBrush(0, 0, 0, MmmjrBotSettings.Instance.OverlayTransparency);
		}
    }
}
