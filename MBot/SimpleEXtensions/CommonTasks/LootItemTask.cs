...
using MmmjrBot.SimpleEXtensions.CachedObjects;
using MmmjrBot.SimpleEXtensions.Global;
using MmmjrBot.SimpleEXtensions.Positions;

namespace MmmjrBot.SimpleEXtensions.CommonTasks
{
	public class LootItemTask : ITask
	{
        ...
        {
            if (!MmmjrBotSettings.Instance.ShouldLoot) return false;
            ...
            var refPos = MmmjrBot.Leader != null ? MmmjrBot.Leader.Position : LokiPoe.Me.Position;

			validItems = allItems.FindAll(i => i.Position.AsVector.Distance(refPos) <= MmmjrBotSettings.Instance.MaxLootDistance);
            ...
            if (pos.Distance > MmmjrBotSettings.Instance.MaxLootDistance)
            {
                GlobalLog.Warn($"[LootItemTask] {pos} is now to far away. [{pos.Distance}/{MmmjrBotSettings.Instance.MaxLootDistance}]");
				_item.Ignored = true;
                _item = null;
                return true;
			}
