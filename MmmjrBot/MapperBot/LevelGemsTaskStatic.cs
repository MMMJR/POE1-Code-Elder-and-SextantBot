...
namespace MmmjrBot
{
    public static class LevelGemsTaskStatic
    {
        private static ILog Log = Logger.GetLoggerInstanceForType();
        private static WaitTimer _levelWait = WaitTimer.FiveSeconds;
        private static bool _needsToUpdate = true;
        private static bool _needsToCloseInventory;
        ...
        public static async Task<bool> Run()
        {
            ...
            if (_needsToUpdate)
            {
                ...
                if (!await Lib.Inventories.OpenInventory())
                {
        ...
        static Func<Inventory, Item, Item, bool> eval = (inv, holder, gem) =>
        {
            ...
            {
                if (MmmjrBotSettings.Instance.MapperGemDebugStatements)
                {
            ...
            if (MmmjrBotSettings.Instance.MapperLevelOffhandOnly)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.MapperUserSkillGemsInOffHands;
                foreach (MmmjrBotSettings.SkillGemEntry g in userSkillGems)
                {
                    gemsToConsider.Add(string.Format("{0} [{1}: {2}]", g.Name, g.InventorySlot, g.SocketIndex));
                }
            }
            else if (MmmjrBotSettings.Instance.MapperLevelAllGems)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.MapperUserSkillGems;
                foreach (MmmjrBotSettings.SkillGemEntry g in userSkillGems)
                {
                    gemsToConsider.Add(string.Format("{0} [{1}: {2}]", g.Name, g.InventorySlot, g.SocketIndex));
                }
        ...
        {
            foreach (string entry in MmmjrBotSettings.Instance.MapperGlobalNameIgnoreList)
            {
