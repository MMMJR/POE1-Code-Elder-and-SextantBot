...
namespace MmmjrBot.Helpers
{
    public static class PartyHelper
    {
        ...
        public static async Task<bool> HandlePartyInvite()
        {
            ...
            {
                MmmjrBot.Log.WarnFormat($"[MmmjrBot] Visible Notifications: {LokiPoe.InGameState.NotificationHud.NotificationList.Where(x => x.IsVisible).ToList().Count}");
                LokiPoe.InGameState.ProcessNotificationEx isPartyRequestToBeAccepted = (x, y) =>
                {
                    var res = y == LokiPoe.InGameState.NotificationType.Party && (string.IsNullOrEmpty(MmmjrBotSettings.Instance.InviteWhiteList) || MmmjrBotSettings.Instance.InviteWhiteList.Contains(x.CharacterName));
                    MmmjrBot.Log.WarnFormat($"[MmmjrBot] Detected {y.ToString()} request from char: {x.CharacterName} [AccountName: {x.AccountName}] Accepting? {res}");
                    return res;
                };
                ...
                MmmjrBot.Log.WarnFormat($"[HandlePartyInvite] Result: {ret}");
