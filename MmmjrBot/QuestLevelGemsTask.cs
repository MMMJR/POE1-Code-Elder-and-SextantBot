...
namespace MmmjrBot
{
    public class QuestLevelGemsTask : ITask
    {
        ...
        Func<Inventory, Item, Item, bool> eval = (inv, holder, gem) =>
        {
            ...
            {
                if (MmmjrBotSettings.Instance.QuestGemDebugStatements)
                {
            ...
            if (MmmjrBotSettings.Instance.QuestLevelOffhandOnly)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.QuestUserSkillGemsInOffHands;
            ...
            else if (MmmjrBotSettings.Instance.QuestLevelAllGems)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.QuestUserSkillGems;
                foreach (MmmjrBotSettings.SkillGemEntry g in userSkillGems)
                {
                    gemsToConsider.Add(string.Format("{0} [{1}: {2}]", g.Name, g.InventorySlot, g.SocketIndex));
                }
        ...
        {
            foreach (string entry in MmmjrBotSettings.Instance.QuestGlobalNameIgnoreList)
            {
