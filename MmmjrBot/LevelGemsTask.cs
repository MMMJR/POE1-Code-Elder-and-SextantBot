...
namespace MmmjrBot
{
    public class LevelGemsTask : ITask
    {
        ...
        Func<Inventory, Item, Item, bool> eval = (inv, holder, gem) =>
        {
            ...
            {
                if (MmmjrBotSettings.Instance.GemDebugStatements)
                {
            ...
            if (MmmjrBotSettings.Instance.LevelOffhandOnly)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.UserSkillGemsInOffHands;
            ...
            else if (MmmjrBotSettings.Instance.LevelAllGems)
            {
                ObservableCollection<MmmjrBotSettings.SkillGemEntry> userSkillGems = MmmjrBotSettings.Instance.UserSkillGems;
                foreach (MmmjrBotSettings.SkillGemEntry g in userSkillGems)
                {
                    gemsToConsider.Add(string.Format("{0} [{1}: {2}]", g.Name, g.InventorySlot, g.SocketIndex));
                }
        ...
        {
            foreach (string entry in MmmjrBotSettings.Instance.GlobalNameIgnoreList)
            {
