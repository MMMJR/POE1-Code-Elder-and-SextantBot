...
namespace MmmjrBot.AutoLogin
{
    public static class AutoLogin
    {
        private static readonly ILog Log = Logger.GetLoggerInstanceForType();

        private static float _currentLoginDelay;
        private static int _currentCharSelectAttempt;

        private static bool _needsLoginReset;
        private static bool _needsCharReset;

        public static void Start()
        {
            _currentLoginDelay = MmmjrBotSettings.Instance.LoginDelayInitial;
            _currentCharSelectAttempt = 0;
            MmmjrBotSettings.Instance.LoginUsingGateway = false;
        ...
        public static async Task<bool> CharacterSelection()
        {
            // Sanity check, to make sure the coroutine is in the expected state.
