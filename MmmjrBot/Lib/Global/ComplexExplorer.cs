...
namespace MmmjrBot.Lib.Global
{
    public class ComplexExplorer
    {
        ...
        public readonly GridExplorer _explorer;
        ...
        public GridExplorer BasicExplorer =>  !World.CurrentArea.IsMap ? _explorer : _explorers[_levelAnchor];

        public ComplexExplorer()
        {
            Settings = ProvideSettings();
            Settings.LogProperties();
            if (Settings.BasicExploration || !World.CurrentArea.IsMap)
            {
        ...
        private async Task<bool> ComplexExploration()
        {
            ...
            if (MmmjrBotSettings.Instance.FastTransition)
            {
        ...
        public void HandleExternalJump()
        {
            GlobalLog.Info("Aqui");
            CombatAreaCache.Current.Explorer._transition = null;
            GlobalLog.Info("Aqui2");
            foreach (var t in CombatAreaCache.Current.AreaTransitions)
            {
                t.Visited = false;
            }
            GlobalLog.Info("Aqui3");
            // Current explorer is ticking on a wrong level, reset it.
            CombatAreaCache.Current.Explorer._disableTick = true;
            CombatAreaCache.Current.Explorer.BasicExplorer.Reset();
            GlobalLog.Info("Aqui33");
            CombatAreaCache.Current.Explorer.ResetLevelAnchor();
            CombatAreaCache.Current.Explorer._disableTick = false;
            GlobalLog.Info("Aqui4");
        }
        ...
        private void InitNewLevel()
        {
            ...
            if(_explorers.ContainsKey(_levelAnchor))
            {
                _explorers.Remove(_levelAnchor);
            }
    ...
    public class ExplorationSettings
    {
        ...
        )
        {
            ...
            FastTransition = MmmjrBotSettings.Instance.FastTransition;
