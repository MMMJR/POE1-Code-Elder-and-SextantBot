...
namespace MmmjrBot.Lib.Global
{
    public class CombatAreaCache
    {
        ...
        public static CombatAreaCache Current
        {
            get
            {
                ...
                if (Caches.TryGetValue(hash, out var cache))
                {
                    ...
                    if(!oldHash)
                    {
                        /*if (MmmjrBotSettings.Instance.SingleUseElderFragments && cache.WorldArea.IsMap)
                        {
                            GlobalLog.Info("AQUIII RESETOU");
                            cache.Explorer.Finished = false;
                            cache.Explorer.BasicExplorer.Reset();
                            cache.Explorer.ResetLevelAnchor();
                            cache.AreaTransitions.Clear();
                            cache.Explorer.Finished = false;
                        }*/
                        oldHash = true;
                    }
        ...
        private void OnTick()
        {
            ...
            {
                if (ItemScanInterval.Elapsed && MmmjrBotSettings.Instance.MapperShouldLoot)
                {
                    WorldItemScan();
                }
        ...
        private void WorldItemScan()
        {
            ...
            foreach (var obj in labels)
            {
                ...
                {
                    if(item.IsElderItem && item.ItemType != InventoryType.Neck)
                    {
                        continue;
                    }
        ...
        private void ProcessMonster(Monster m)
        {
            ...
            if (m.Metadata.Contains("TentaclePortal") || m.Name.Contains("Portal") || m.Name.Contains("Formless") || m.Name.Contains("Witness"))
            {
                return;
            }

            if(m.Metadata == "Metadata/Monsters/AtlasBosses/ZanaElder" || m.Metadata == "Metadata/Monsters/AtlasBosses/TheShaperBossElderEncounter" || m.Metadata == "Metadata/Monsters/ArcticBreath/ArcticBreathSkull" || m.Metadata == "Metadata/Monsters/RaisedSkeletons/RaisedSkeletonStandard")
            {
                return;
            }
        ...
        private void ProcessTransition(AreaTransition t)
        {
            ...
            var pos = t.WalkablePosition(10, 50);
