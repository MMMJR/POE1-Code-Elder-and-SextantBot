...
namespace MmmjrBot.Lib
{
    public static class PlayerAction
    {
        ...
        public static async Task<bool> Interact(NetworkObject obj, Func<bool> success, string desc, int timeout = 6000)
        {
            if (obj == null)
            {
        ...
        public static async Task<bool> TpToTown(bool forceNewPortal = false, bool repeatUntilInTown = true)
        {
            ...
            if (forceNewPortal || (portal = PortalInRangeOf(100)) == null)
            {
        ...
        public static Portal PortalInRangeOf2(int range)
        {
            return LokiPoe.ObjectManager.Objects
                .Closest<Portal>(p => p.IsTargetable && p.Distance <= range && p.PathDistance() <= range + 3);
        }
